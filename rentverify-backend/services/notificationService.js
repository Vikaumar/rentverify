...
/**
 * Sends a real-world WhatsApp message using Twilio
 */
async function sendWhatsApp(to, body) {
  if (!to) return;
  const rawPhone = to.trim();
  const targetPhone = rawPhone.startsWith('whatsapp:') ? rawPhone : `whatsapp:${rawPhone}`;
  const senderNumber = TWILIO_WHATSAPP_NUMBER || 'whatsapp:+14155238886';
...
function logSimulatedSMS(to, body, warn = '') {
  ...
  if (line) console.log(`│ ${line.padEnd(54)} │`);
  console.log('└────────────────────────────────────────────────────────┘\n');
}
  ...
  return `
    ...
    <head>
      ...
      <style>
        ...
        .btn { display: inline-block; padding: 14px 28px; background: #6c63ff; color: #ffffff !important; text-decoration: none; font-weight: bold; border-radius: 8px; box-shadow: 0 4px 6px rgba(108,99,255,0.2); }
    ...
    <body>
      <div class="card">
        ...
        <div class="content">
          ...
          <p>Your property host has initiated your secure check-in verification for your upcoming stay.</p>
          <div class="dates-box">
            <div class="dates-title">Stay Duration</div>
            <div class="dates-values">${checkinDate} &mdash; ${checkoutDate}</div>
          </div>
          <p>Please complete this quick verification by clicking the secure link below.</p>
          <div class="btn-container">
            <a href="${secureLink}" class="btn">Verify My Identity</a>
          </div>
          <p style="font-size: 13px; color: #7f8c8d;">If the button doesn't work, copy and paste this URL:<br>
          <a href="${secureLink}" style="color: #6c63ff; word-break: break-all;">${secureLink}</a></p>
        </div>
        <div class="footer">
          &copy; 2026 Property Guardian. All rights reserved.
        </div>
      </div>
    </body>
    </html>
  ...
  return `
    ...
    <head>
      ...
      <style>
        ...
        .btn-container { text-align: center; margin: 30px 0; }
        .btn { display: inline-block; padding: 12px 24px; background: #00b894; color: #ffffff !important; text-decoration: none; font-weight: bold; border-radius: 8px; }
        .footer { padding: 20px; text-align: center; font-size: 12px; color: #8898aa; background-color: #fafbfc; border-top: 1px solid #eef2f5; }
      </style>
    </head>
    <body>
      <div class="card">
        ...
        <div class="content">
          <p><strong>Guest Verification Completed</strong></p>
          <p>A new guest has completed their digital verification flow.</p>
          <table class="info-table">
            <tr><td class="info-label">Guest Name</td><td class="info-value">${guestName}</td></tr>
            <tr><td class="info-label">Reference ID</td><td class="info-value" style="font-family: monospace;">${refCode}</td></tr>
            <tr><td class="info-label">Submitted At</td><td class="info-value">${new Date().toLocaleString()}</td></tr>
          </table>
          <div class="btn-container">
            <a href="${reviewLink}" class="btn">Open Admin Dashboard</a>
          </div>
        </div>
        <div class="footer">&copy; 2026 Property Guardian. Admin Alert System.</div>
      </div>
    </body>
    </html>
  `;
...
function getDecisionTemplate(guestName, isApproved, rejectionReason, loginLink) {
  ...
  const statusMessage = isApproved
    ? 'Great news! Your check-in documents have been successfully verified and approved.'
    : `We were unable to approve your check-in documents. Reason: <strong style="color: #d63031;">"${rejectionReason}"</strong><br><br>Please re-submit using your original link.`;
  ...
  return `
    ...
    <body>
      <div class="card">
        <div class="header"><h1>${statusTitle}</h1></div>
        <div class="content">
          <div style="font-size: 18px; font-weight: 600; margin-bottom: 10px;">Hello ${guestName},</div>
          <div class="status-card">${statusMessage}</div>
          ${!isApproved ? `<div class="btn-container"><a href="${loginLink}" class="btn">Update Verification</a></div>` : ''}
        </div>
        <div class="footer">&copy; 2026 Property Guardian. All rights reserved.</div>
      </div>
    </body>
    </html>
  `;
